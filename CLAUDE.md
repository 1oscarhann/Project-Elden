...
- **Terrain tiles are generated by `tools/gen_terrain_tiles.py`** into
  `assets/tiles/island_terrain.png` (16x16, 6 terrain rows x 4 variants). The **palette and
  motifs come from the owner's chosen top-down grass/dirt/stone tileset**, kept in
  `docs/reference/tileset_reference_preview.webp`.
  **That reference is a lossy 1800x1200 marketing preview, not the source asset.** Edge
  run-lengths measure 1,2,3,4,6,7,8,10... with no consistent multiple, and a downscale/upscale
  error sweep rises monotonically with no minimum at any integer scale — i.e. it was resampled
  non-integer, so **there is no pixel grid to recover and its tiles cannot be sliced cleanly**.
  Colours survive resampling perfectly, so they were sampled and the tiles redrawn crisp at 16px.
  **If the original PNG/zip ever turns up, slice it properly** — keep the row order and nothing
  else in the project changes.
- **What that sheet actually contains**, for whoever slices it later: its grass and dirt base
  tiles are **flat single colours** (the detail is all in cliff/edge autotile pieces), and it has
  **no ocean tile at all** — only blue wave overlays on transparent. It also has **no sand**, so
  beaches use its dirt brown. Sampled palette: grass `#7f9f55` (leafy highlight `#9fb455`),
  dirt `#9f5f3f`, rock `#b47f3f`, water `#5574bf` (light `#5f7fc9`); deep water is that blue
  darkened to `#445ea0`.
