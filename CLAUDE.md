...
**Phase 2 complete.** Next up: `docs/phases/phase03_daynight.md`.

### Phase 2 notes

- **The dungeon pack (169442) is DROPPED** — wrong art style for a sunny island. Its zip is still
  in `raw_assets/` but nothing imports it and it is out of `CREDITS.md`. **Open gap:** it was the
  planned source for the campfire flame (Phase 4) and doors/chests/interior tiles (Phase 8).
  Those phases need replacement art.
- **Terrain tiles are placeholder programmer-art**, generated by `tools/gen_terrain_tiles.py`
  into `assets/tiles/island_terrain.png` (16x16, 6 terrain rows x 4 variants). No remaining pack
  has ground tiles. To swap in real art: replace the PNG, rebuild the TileSet, keep the row
  order — nothing else changes, because the generator only ever emits `Terrain` enum values.
